...
var dbstream = function () { 
	...
	/**
	 * Query objects by using specified query options which meets the limitation options.
	 ...
	 the value of the 'limit'. The objects in the array are organized by the 'benchmark' property in ascending order.

	 * @example
	 Query the latest 10 objects.
	 {
		"limit": 10
	 }
	...
	this.queryByLimitationOptions = function(colName, queryOptions, limitationOptions, callback){
		...
		if(!limitationOptions.start && !limitationOptions.duration){
			process.nextTick(function(){ callback(new Error("The start or duration is required."));});
		}
		...
		*/
		...
		queryObject[limitationOptions.benchmark] = {'$gt': start, '$lte': end};
		...
		var sortObject = {};
		sortObject[limitationOptions.benchmark] = bQueryLatestObjects ? -1 : 1;
